...
const displayedSubjects = computed(() =>
  (getDisplaySubjects(props.academy?.subjects ?? []) ?? []).filter((s): s is string => s != null && s !== '')
)

/** 유효한 연령 그룹만 필터링하고 표준 순서로 정렬하여 표시 */
const displayedAgeGroups = computed(() => {
  if (!props.academy?.age_group) return []
  const validGroups = props.academy.age_group.filter((a): a is string => a != null && a !== '' && isValidAgeGroup(a))
