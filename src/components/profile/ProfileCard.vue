            ...
            <template v-else-if="row.type === 'number'">
              <span class="input-underline-group" @click.stop>
                ...
                <input
                  ...
                  :min="row.min"
                  :max="row.max"
                  @input="row.onInput(Number(($event.target as HTMLInputElement).value))"
                />
...
const emit = defineEmits<{
  ...
  'update:childForm': [v: { name: string; age: number; gender: 'male' | 'female' | null }]
}>()
    ...
    return [
      {
        ...
        onInput: (v) => emit('update:profileForm', { ...props.profileForm!, nickname: v }),
      },
    ]
  }
  if (props.variant === 'child' && (props.child || props.isEditing)) {
    ...
    return [
      ...
      },
      {
        key: 'age',
        label: '나이',
        display: c != null && typeof c.age === 'number'
      ...
      },
      {
        key: 'gender',
        label: '성별',
        display: c ? (c.gender === 'male' ? '남아' : c.gender === 'female' ? '여아' : '—') : '',
        type: 'gender',
...
const canSaveCurrent = computed(() => {
  if (props.variant === 'parent' || props.variant === 'student') return props.canSaveProfile ?? false
